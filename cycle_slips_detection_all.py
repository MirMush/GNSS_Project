...
def get_receiver_latlon(filepath):
    """Read receiver lat/lon from RINEX header APPROX POSITION XYZ."""
    try:
...
# ---------------------------------------------------------------------------
# Loop over all RINEX files in data/
# ---------------------------------------------------------------------------
rinex_files = sorted(glob.glob("data/*.24o"))
print(f"Found {len(rinex_files)} RINEX files in 'data/'")

all_results = {}  # station -> {results_l1, results_gf, lat, lon}

for rinex_path in rinex_files:

    station = os.path.basename(rinex_path)[:4].upper()
    print(f"\n{'='*50}\nProcessing station: {station} ({rinex_path})\n{'='*50}")

    lat, lon = get_receiver_latlon(rinex_path)

    try:
        rnx = rinexReader(rinex_path)
        rnx.readFile(
            readConst=["G", "E"],
    ...
    # ---------------------------------------------------------------------------
    # Save results for mapping script (one pkl per station)
    # ---------------------------------------------------------------------------
    pkl_path = os.path.join(OUT_DIR, f"{station}_slip_results.pkl")
    with open(pkl_path, "wb") as f:
        pickle.dump({
            "plot_data_gf": plot_data_gf,
            "file_start":   rnx.fileStart,
            "file_end":     rnx.fileEnd,       # <-- added for time-window aggregation
            "lat":          lat,
            "lon":          lon,
        }, f)
    print(f"Saved slip results to {pkl_path}")

    # store for combined summary at the end
    all_results[station] = {
        "results_l1": results_l1,
        "results_gf": results_gf,
        "lat": lat,
        "lon": lon,
    }
    ...
    # ---------------------------------------------------------------------------
    print("\n" + "=" * 40)
    print(f"Cycle slips found — {station}")
    print("=" * 40)
    ...
    fig.suptitle(f"Cycle Slip Timeline — {station} — red marks = detected slips", fontsize=11)
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, f"{station}_timeline.png"), dpi=120, bbox_inches="tight")
    plt.close()
    ...
    ax.set_title(f"Cycle Slips per Satellite: L1-only vs Geometry-Free — {station}")
    ax.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(OUT_DIR, f"{station}_barchart.png"), dpi=120, bbox_inches="tight")
    plt.close()

# ---------------------------------------------------------------------------
# Combined summary across all stations
# ---------------------------------------------------------------------------
with open(os.path.join(OUT_DIR, "all_slip_results.pkl"), "wb") as f:
    pickle.dump(all_results, f)

print("\n" + "=" * 60)
print(f"{'STATION':<8}  {'L1 total':>10}  {'GF total':>10}  {'lat':>7}  {'lon':>8}")
print("=" * 60)
for station, res in all_results.items():
    total_l1 = sum(res["results_l1"].values())
    total_gf = sum(res["results_gf"].values())
    lat_str  = f"{res['lat']:.1f}" if res["lat"] else "N/A"
    lon_str  = f"{res['lon']:.1f}" if res["lon"] else "N/A"
    print(f"{station:<8}  {total_l1:>10}  {total_gf:>10}  {lat_str:>7}  {lon_str:>8}")